...
struct ptuv_pickup_plan{
    void (*_execute)(ptuv_pickup_plan* plan);
    void execute(){
        _execute(this);
    }
...
void LINEAR_pickup(ptuv_pickup_plan* plan){
    plan->flag_changed = 0;
    for (int i = 0; i < 3; i++) {
...
ptuv_pickup_plan create_ptuv_pickup_plan(ptuv_config* conf){
    ptuv_pickup_plan plan;
...
void* ptuv_thread(void *arg){

    auto conf = reinterpret_cast<ptuv_config*>(arg);
    conf->running = 1;

    ptuv_pickup_plan pickup_plan = create_ptuv_pickup_plan(conf);
