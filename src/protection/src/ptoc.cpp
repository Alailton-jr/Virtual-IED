...
struct ptoc_pickup_plan {
    void (*_execute)(ptoc_pickup_plan* plan);
    void execute() {
        _execute(this);
    }
...
void PHASE_pickup(ptoc_pickup_plan* plan){
    plan->flag_changed = 0;
    for (int i = 0; i < 3; i++) {
...
void NEUTRAL_pickup (ptoc_pickup_plan* plan){
    plan->flag_changed = 0;
...
ptoc_pickup_plan create_ptoc_pickup_plan(ptoc_config* conf){

    ptoc_pickup_plan plan;
...
void* ptoc_thread(void* arg){
    ...
    ptoc_pickup_plan pickup_plan = create_ptoc_pickup_plan(conf);
    trip_time_plan trip_plan = create_trip_time_plan(&pickup_plan.pickup_flag, conf->time_dial, conf->curve, trip_time_plan::INVERSE_TIME, pickup_plan.pickup_flag.size(), conf->module, conf->pickup);
